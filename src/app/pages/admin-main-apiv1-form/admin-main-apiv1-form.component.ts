import { Component, Input, OnInit, OnChanges, SimpleChanges } from '@angular/core'; // Añade OnChanges y SimpleChanges
...
@Component({
  selector: 'app-admin-main-apiv1-form', // Este es tu selector
  ...
  styleUrls: ['./admin-main-apiv1-form.component.css']
})
export class AdminMainApiv1FormComponent implements OnInit, OnChanges { // Implementa OnChanges
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['initialData'] && changes['initialData'].currentValue && this.operation === 'edit') {
    ...
    if (changes['operation'] && this.myForm) {
        ...
        if (changes['operation'].currentValue === 'edit' && this.initialData) {
            this.populateForm(this.initialData);
        }
    }
  }
  ...
  defineFormStructure(): void {
    ...
    this.formStructure = [
      {
        key: 'Pais', type: 'radio', label: 'Pais', required: true,
        options: [ { value: 'MX', name: 'MX' }, { value: 'COL', name: 'COL' }, { value: 'GTM', name: 'GTM' }] as RadioButtonOption[]
      },
      { key: 'CodigoDistribuidor', type: 'text', label: 'Codigo DS', placeholder: 'Codigo', required: true },
      { key: 'Contrasena', type: 'text', label: 'Contraseña', placeholder: 'Contraseña', required: true },
      { key: 'Correo', type: 'email', label: 'Correo Electrónico', placeholder: 'correoDS@correo.com', required: true },
      { key: 'CorreosCopia', type: 'text', label: 'Correos Copia', placeholder: 'Separar por ;' },
      {
        key: 'Planta', type: 'MultipleCheckbox', label: 'Almacenes - Stocks', required: true,
        options: [
           { value: 1010, name: 'MX-Guadalajara' }, { value: 1011, name: 'MX-Algarín' },
           { value: 1012, name: 'MX-Monterrey' }, { value: 1013, name: 'MX-Gustavo Baz' },
           { value: 1014, name: 'MX-Torreón' }, { value: 1210, name: 'COL-Colombia' },
           { value: 1211, name: 'COL-RIC' }, { value: 1310, name: 'GTM-Guatemala' }
  ...
  createFormGroup(): FormGroup {
    ...
    this.formStructure.forEach(field => {
      ...
      const initialValue = (this.operation === 'edit' && this.initialData && this.initialData[field.key] !== undefined)
                           ? this.initialData[field.key]
                           : (field.type === 'toggle' || field.type === 'checkbox' ? false : (field.type === 'MultipleCheckbox' ? [] : '')); // Valor por defecto
  ...
  onActualFormSubmit(): void {
    if (this.myForm.valid) {
      ...
      if (this.operation === 'edit' && this.initialData && this.initialData.id) { // Asume que tienes un 'id' o identificador
      ...
      // Aquí procesarías dataToSubmit (enviar a un servicio, etc.)
      // Probablemente querrás emitir un @Output() con dataToSubmit
    } else {
